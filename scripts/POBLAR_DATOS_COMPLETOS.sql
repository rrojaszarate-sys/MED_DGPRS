...
DELETE FROM proveedores WHERE id LIKE '2000%';
...
SELECT
  ...
  8000,
  CASE
    WHEN RANDOM() > 0.15 THEN 'disponible'
    WHEN RANDOM() > 0.5 THEN 'cuarentena'
    ELSE 'vencido'
  END,
  true,
  NOW(),
...
SELECT
  ...
  l.centro_id,
  (ARRAY['entrada', 'salida', 'ajuste'])[FLOOR(RANDOM() * 3 + 1)],
  FLOOR(RANDOM() * 200 + 10)::INTEGER,
  l.cantidad_actual,
  l.cantidad_actual + FLOOR(RANDOM() * 200 - 100)::INTEGER,
  (ARRAY['Compra regular', 'Dispensación a pacientes', 'Ajuste de inventario', 'Transferencia entre centros'])[FLOOR(RANDOM() * 4 + 1)],
